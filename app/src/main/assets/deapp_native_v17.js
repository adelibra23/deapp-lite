(function () {
  if (window.__DEAPP_NATIVE_V17__) {
    if (window.__DEAPP_NATIVE_V17__.refresh) window.__DEAPP_NATIVE_V17__.refresh();
    return true;
  }
  ...
  root.classList.add('deapp-native-shell', 'deapp-native-v17');
  if (isProfilePage) root.classList.add('deapp-native-profile');

  const style = document.createElement('style');
  style.id = 'deapp-native-v17-style';
  style.textContent = `
    ...
    /* v1.7 — shell native stabil: gesture sheet dan refresh tidak saling berebut; tap tetap tanpa flash browser. */
  ...
  function injectAboutSettings() {
    ...
    row.innerHTML = '<span style="font-size:20px;line-height:1">ⓘ</span><span><b>Tentang aplikasi</b><small>Deapp Lite untuk Android</small></span><span class="deapp-version-pill">v1.7.0-lite</span>';
    row.addEventListener('click', function(){
      nativeTap();
