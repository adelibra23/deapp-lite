(function () {
  if (window.__DEAPP_NATIVE_V16__) {
    if (window.__DEAPP_NATIVE_V16__.refresh) window.__DEAPP_NATIVE_V16__.refresh();
    return true;
  }
  ...
  root.classList.add('deapp-native-shell', 'deapp-native-v16');
  if (isProfilePage) root.classList.add('deapp-native-profile');

  const style = document.createElement('style');
  style.id = 'deapp-native-v16-style';
  style.textContent = `
    ...
    /* v1.6 — tidak ada flash/ripple/pressed state ala browser. Status aktif tetap boleh berubah setelah navigasi. */
  ...
  function injectAboutSettings() {
    ...
    row.innerHTML = '<span style="font-size:20px;line-height:1">ⓘ</span><span><b>Tentang aplikasi</b><small>Deapp Lite untuk Android</small></span><span class="deapp-version-pill">v1.6.0-lite</span>';
    row.addEventListener('click', function(){
      nativeTap();
